...
let textLines = `Самсон: Уж поверь моему слову, Грегори, мы бобов разводить не станем.
...
let reg = new RegExp(`(${roles.join('|')})+\:`, 'gim');
...
let generator = gen();
let numStr = textLines.replace(reg, function (match) {
  return match + ' ' + generator.next().value + ')';
});

for(let text of roles) {  
  let regRoles = new RegExp('((' + text + ')+\:)(.+[\.\?\!])', 'gi');
  console.log(text + ':');
  numStr.replace(regRoles, function(match, p1, p2, p3) {
    console.log(p3);
    return match;
  })
};
